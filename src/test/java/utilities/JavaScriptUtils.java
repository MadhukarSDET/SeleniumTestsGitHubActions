...
public class JavaScriptUtils {
	...
	public static boolean waitForPageLoad(WebDriver driver) {
		...
		String[] classNameArray = new Throwable().getStackTrace()[1].getClassName().split("\\.");
		String pageName = classNameArray[classNameArray.length - 1];
		...
		for (int i = 0; i < 30; i++) {
			...
			if (state.equals("complete")) {
				isPageLoaded = true;
				System.out.println(pageName + " Loaded Successfully!!");
				break;
			} else {
				System.out.println(pageName + " Failed to load!!");
			}
			try {
				Thread.sleep(1000);
